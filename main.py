...
def run():
    ...
    assignmentFolder = os.getcwd() + '/DEV' #goes through dev folder, loops thru directories to go thru all docs
    for directories in os.listdir(assignmentFolder):
        ...
        for file in os.listdir(os.path.join(assignmentFolder, directories)):
            ...
            if (fileCount == 1000):  #cs/se constraint where we need to write index to file in batches 
                fileCount = 0 #every 1k files indexed, write index into file, reset index 
                ind = writeToFile(frequencies, len(bitIndexes))
                bitIndexes.append(ind)
                frequencies = {}

            with open(os.path.join(subfolder, file), 'r') as j: #open file 
                jsonContentDictionary = json.load(j)
                currURL = ''

                for key, value in jsonContentDictionary.items():#looking at the json file....
                    if key == 'url': #if key is url store url and put it into dictionary of doc IDs (assigns url an ID)
                        currURL = value
                        validJsonCounter = validJsonCounter + 1
                        docID[str(idCounter)] = currURL

                    if key == 'content':#if key is content, where we do processing/indexing 
                        soup = BeautifulSoup(value, 'html.parser') #html parser fixes broken html

                        # find and calculate html tag weights (g(d)) 
                        header = soup.find_all(re.compile('^h[1-6]$')) #regex to collect everything in between header tags
                        headerText = [] #want only text and no other html tags 
                        for h in header: #loop thru list of header text
                            if len(h) > 0: #make sure its not empty
                                text = re.findall(r'>(.*)<', str(h)) #use regex to find in between 
                                for t in text:
                                    t = t.replace('\n', '')#changed new line to space b/c realized that new lines between text gave errors
                                    while t.find('<') != -1:  #fix nested html tags since regex goes thru 1 pair of text 
                                        start = t.find('<') #loop goes thru start and end of html tags to rid of it 
                                        end = t.find('>', start)
                                        t = t[:start] + t[end + 1:]
                                    if len(t) > 0:
                                        headerText.append(t)
                        ...
                        del header
                        headerTokens = [] #lines 78-83 tokenize each text and appends it to a list holding tokens that appear in headers 
                        for t in headerText:
                            hTokens = tokenizer(t)
                            for t2 in hTokens:
                                if len(t2) > 1:
                                    headerTokens.append(t2)
                        del headerText #rid of memory 

#lines 87-95 find corresponding text of tags like for header..didntc call method for header b/c needed extra regex to find headers 1-6  
                        ...
                        tokens = tokenizer(soup.text)  #tokenize the document

                        for t in tokens:  #goes through list of tokens and stems the word 
                            stemmedT = porterStem.stem(t) # used porter stemmer lib
                            if stemmedT in frequencies.keys(): #create dict with token as key pointing to another dict with docID as key: list where [0] is term frequency of tf-ifd [1] is html tag weight 
                                if str(idCounter) in frequencies[stemmedT].keys():
                                    frequencies[stemmedT][str(idCounter)][0] += 1
                                else:
                                    frequencies[stemmedT][str(idCounter)] = [1, 0] #html tag weight initialized to 0
                            else:
                            ...
                            if t in titleTokens: #lines 114-133 where we initialze the html tag weights 
                ...
                fileCount += 1 #increment for writing index to files
                totalFileCount += 1 #incremeent for tf-ifd calculation
                idCounter += 1 #for docID

    if len(frequencies) != 0: #after looping through all the files, check if index has anything left ..
        ind = writeToFile(frequencies, len(bitIndexes)) #write index into file again and append the pointer index tot he pointer index list
        bitIndexes.append(ind)
#We write the dictionary holding the urls and docIDs into file so we can use it in query.py
    with open('docID.json', 'w+') as d:
        json.dump(docID, d)
        d.close()

    mergeFiles(bitIndexes, totalFileCount) #call merge files, take all the files we created for index, merge 1 file, where we calc tf-idf for each doc under each token
    lengthNormalization() #deals cosine similarity, normalizes tf-idf scores 
    writeCacheWords() #to be under time constrainst , function writes cache words from index into file that query will look into
